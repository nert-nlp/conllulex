...
def unrender(rendered, toks):
    ...
    if len(toks) == 1:
        ...
        reMarkup = (
            rf"^(?P<t0>{re.escape(toks[0])})((?P<L0>\|[^ _~]+)?[ ~]|_)"
        ...
        reMarkup += (
            rf"(?P<t{len(toks) - 2}>{re.escape(toks[-2])})"
    ...
    for i in range(len(toks)):
        ...
        if i == 0:
            ...
            l = rendered[
                matches.end(f"L{i - 1}" if labels_at_end[-1] is not None else f"t{i - 1}") : matches.start(f"t{i}")
            ]
